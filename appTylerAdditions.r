  ...
  sidebarLayout(
    sidebarPanel(
      ...
      tags$div(
        ...
        "The Predictor variables are below:"
      ),
      tags$div(
...
These are Games Played, Team, Runs, Double, Triples, Home Runs, Walks, Strike Outs, Stolen Base, Birth Country, Weight, Height, Batting Hand, Throwing Hand, Position, Salary, School Playing and Awards. We have left out the Player ID, At bats and hits from the model since they are directly related to the batting average and we wanted to avoid overfitting. 
...
The numerical values are considered for the correlation matrix since the categorical variables are not supported.",br() 
  ...
  output$des <- renderText({
    paste(br(),"From the correlation matrix, we can see that the variables Games Played, Runs, Doubles, Triples, Home Runs, Walks, Strike Outs, Stolen Bases and Salary have the greatest correlation with the batting average.")
  })
